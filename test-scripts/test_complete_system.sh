...
DID_1=$(echo "$REGISTER_1" | jq -r '.did // empty')
OBJECT_ID_1=$(echo "$REGISTER_1" | jq -r '.object_id // empty')
CREDENTIAL_1=$(echo "$REGISTER_1" | jq -r '.credential_jwt // empty')
# Extract credential ID from JWT (it's in the 'vc.id' claim)
CREDENTIAL_ID_1=$(echo "$CREDENTIAL_1" | cut -d'.' -f2 | base64 -d 2>/dev/null | jq -r '.vc.id // empty' 2>/dev/null || echo "")
...
DID_2=$(echo "$REGISTER_2" | jq -r '.did // empty')
OBJECT_ID_2=$(echo "$REGISTER_2" | jq -r '.object_id // empty')
...
DID_3=$(echo "$REGISTER_3" | jq -r '.did // empty')
OBJECT_ID_3=$(echo "$REGISTER_3" | jq -r '.object_id // empty')
...
if [ -n "$DID_1" ]; then
    ...
    DOC_ID=$(echo "$RESOLVE_1" | jq -r '.did_document.id // empty')
    
    if [ -n "$DOC_ID" ] && [ "$DOC_ID" != "null" ]; then
        pass "DID 1 resolved"
        
        # Count verification methods
        VM_COUNT=$(echo "$RESOLVE_1" | jq '.did_document.verificationMethod | length // 0' 2>/dev/null || echo "0")
        echo "  Document ID: $DOC_ID"
        echo "  Verification Methods: $VM_COUNT"
        
        # Show first verification method
        echo "$RESOLVE_1" | jq -r '.did_document.verificationMethod[0].id // empty' 2>/dev/null | \
...
if [ -n "$DID_3" ]; then
    ...
    VM_COUNT_BEFORE=$(echo "$RESOLVE_BEFORE" | jq '.did_document.verificationMethod | length // 0' 2>/dev/null || echo "0")
    ...
    if [ "$ROTATE_SUCCESS" == "true" ]; then
        ...
        VM_COUNT_AFTER=$(echo "$RESOLVE_AFTER" | jq '.did_document.verificationMethod | length // 0' 2>/dev/null || echo "0")
